# -*- coding: utf-8 -*-
"""
Main script to train and export GPR Forward models for the Holzapfel Material
...
for ii in range(0,len(TEST_MODE_list)):
    ...
    for kk in range(0, len(num_train_list)):
        ...
        ####### GPR Model ##################
        length_scales = np.ones(NUM_FEATURES)
        lscale_bounds = (L_ScaleBound, U_ScaleBound)
        kernel =  RBF(length_scales,length_scale_bounds = lscale_bounds) #Matern(length_scales)
        
        model = GaussianProcessRegressor(kernel=kernel, alpha= ALPHA, random_state=None,n_restarts_optimizer=10)
        ...
        export.stress_strain(Y_val, fX_val, 1, lc_stats['MAE_val'][-1], StrainObservs, star_set, ax1, ax2, final = True) 
